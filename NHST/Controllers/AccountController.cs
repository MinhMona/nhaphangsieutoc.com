...
namespace NHST.Controllers
{
    public class AccountController
    {
        ...
        public static string UpdateFeeRieng(int ID, string FeeBuyPro, string FeeTQVNPerWeight, double Currency)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_Account.Where(ac => ac.ID == ID).FirstOrDefault();
        ...
        public static tbl_Account GetByDathangID(int DathangID)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_Account acc = dbe.tbl_Account.Where(a => a.ID == DathangID).FirstOrDefault();
                if (acc != null)
                    return acc;
                else
                    return null;
        ...
        public static tbl_Account GetBySalerID(int SalerID)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_Account acc = dbe.tbl_Account.Where(a => a.ID == SalerID).FirstOrDefault();
                if (acc != null)
                    return acc;
                else
                    return null;
        ...
        public static List<tbl_Account> GetAllWithStaff(int RoleID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Account> cs = new List<tbl_Account>();
                cs = dbe.tbl_Account.Where(c => c.RoleID == RoleID).OrderBy(c => c.ID).ToList();
                return cs;
            }
        }
        //New
            ...
            using (NHSTEntities dbe = new NHSTEntities())
            {
                ...
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        string url = "http://tt.NHẬP HÀNG SIÊU TỐC.media/paymentservice.asmx/UpdateStatus?UID=4&Key=NHẬP HÀNG SIÊU TỐCsms-autobanking&PayID=" + HpID + "";
