...
namespace NHST.manager
{
    public partial class OrderList : System.Web.UI.Page
    {
        ...
        public void pagingall(List<OrderGetSQL> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    if (roleId == 9)
                    {
                        hcm.Append("<select disabled name=\"\" onchange=\"ChooseDathang('" + item.ID + "', $(this))\" id=\"\">");
                    }
                    else
                    {
                    ...
                    hcm.Append("<td class=\"no-wrap\">" + PJUtils.IntToRequestAdminNew_Thang(item.Status) + "</td>");
                    hcm.Append("<td>");
        ...
        public static string UpdateStaff(int OrderID, int StaffID, int Type)
        {
            ...
            if (obj_user != null)
            {
                if (obj_user.RoleID == 0 || obj_user.RoleID == 2 || obj_user.RoleID == 9)
                {
