                        ...
                        if (od != null)
                        {
                            ...
                            if (setNoti != null)
                            {
                                ...
                                if (setNoti.IsSendEmailUser == true)
                                {
                                    try
                                    {
                                        PJUtils.SendMailGodaddy("admin@nhaphangsieutoc.com", "Nhaphang@123", AccountController.GetByID(Convert.ToInt32(od.UID)).Email,
                                            "Thông báo tại NHẬP HÀNG SIÊU TỐC", "Đơn hàng: " + MainOrderID + " có sản phẩm bị hết hàng.", "");
                                    }
                                    catch
                                    {
                    ...
                    if (listorder.Count > 0)
                    {
                        ...
                        if (Deposit > 0)
                        {
                            if (Deposit > newdeposit)
                            {
                                    ...
                                    if (setNoti.IsSendEmailUser == true)
                                    {
                                        try
                                        {
                                            PJUtils.SendMailGodaddy("admin@nhaphangsieutoc.com",
                                                "Nhaphang@123", AccountInfoController.GetByUserID(Convert.ToInt32(mainorder.UID)).Email,
                                ...
                                //try
                                //{
                                //    PJUtils.SendMailGodaddy("cskh@1688pgs.vn", "1688pegasus", AccountInfoController.GetByUserID(Convert.ToInt32(mainorder.UID)).Email,
